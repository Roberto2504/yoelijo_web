...
class ProductStore {
  ...
  @action findProduct = async (seeker) => {
    try {
      Loader.toggleLoader();
  ...
  @action deleteOrderItem = async (_id) => {
    try {
      Loader.toggleLoader();
      const order = this.order;
      this.order = order.filter(product => product._id !== _id);
    } catch (err) {
      SnackBarStore.setMessage(err.message);
    } finally {
      Loader.toggleLoader();
  ...
  @action exportProduct = async () => {
    try {
      Loader.toggleLoader();
      const { status, data } = await axios.post(`${apiUrl}/product/exportProduct`, this.order);
      runInAction('reload product', () => {
        console.log(data);
      });
      SnackBarStore.setMessage('El producto ha sido eliminado correctamente');
    ...
    return new Promise(async (resolve, reject) => {
      try {
        Async.eachSeries(products, async (product, callback) => {
          try {
            const { status, data } = await axios.post(`${apiUrl}/apiUrl}product/createProduct`, product);
            callback(null);
          } catch (err) {
            callback(err);
          }
  ...
  @action searchProduct = async (code) => {
    try {
      Loader.toggleLoader();
      const { data } = await axios.get(`${apiUrl}/product/search/${code}`);
      runInAction('reload product', () => {
  ...
  @action getProducts = async () => {
    try {
      ...
      const { data } = await axios.get(`${apiUrl}/product/getProducts/${info.page}/${info.limit}`);
      runInAction('reload product', () => {
        this.products = data.response;
        this.pageProducts += 1;
  ...
  @action getMoreProducts = async () => {
    try {
      ...
      Loader.toggleLoader();
      const { status, data } = await axios.get(`${apiUrl}/product/getProducts/${info.page}/${info.limit}`);
      runInAction('reload product', () => {
